...
namespace PokeAutobuilderTests
{
    ...
    public class SmartPokemonMultiplierTests
    {
        ...
        public void SingleType_PopulatesDefenseAndAttackFromDamageRelations()
        {
            ...
            var pokemon = TestFixtures.MakePokemon("basic-mon", types: [t]);
        ...
        {
            Type a = TestFixtures.MakeType("test-dual-a", doubleDamageFrom: ["test-target"]);
            Type b = TestFixtures.MakeType("test-dual-b", doubleDamageFrom: ["test-target"]);

            var pokemon = TestFixtures.MakePokemon("dual-weak-mon", types: [a, b]);
        ...
        {
            Type resist = TestFixtures.MakeType("test-resist-type", halfDamageFrom: ["test-target"]);
            Type weak = TestFixtures.MakeType("test-weak-type", doubleDamageFrom: ["test-target"]);

            var pokemon = TestFixtures.MakePokemon("neutral-mon", types: [resist, weak]);
        ...
        {
            Type immune = TestFixtures.MakeType("test-immune-type", noDamageFrom: ["test-target"]);
            Type weak = TestFixtures.MakeType("test-weak-type-2", doubleDamageFrom: ["test-target"]);

            List<Type> types = immuneFirst ? [immune, weak] : [weak, immune];
            var pokemon = TestFixtures.MakePokemon("immune-mon", types: types);
        ...
        public void DualType_AttackEffectivenessUsesMaxNotStacking()
        {
            ...
            Type a = TestFixtures.MakeType("test-atk-a", doubleDamageTo: ["test-target"]);
            Type b = TestFixtures.MakeType("test-atk-b", doubleDamageTo: ["test-target"]);

            var pokemon = TestFixtures.MakePokemon("dual-attack-mon", types: [a, b]);
        ...
        public void Ability_Levitate_OverridesInnateGroundWeakness()
        {
            ...
            Type heavy = TestFixtures.MakeType("test-heavy", doubleDamageFrom: ["ground"]);

            var withoutLevitate = TestFixtures.MakePokemon("heavy-mon", types: [heavy]);
            Assert.Equal(2.0, withoutLevitate.GetResistance("ground"));

            var withLevitate = TestFixtures.MakePokemon(
                "levitate-mon",
                types: [heavy],
        ...
        {
            Type plain = TestFixtures.MakeType("test-plain");

            var pokemon = TestFixtures.MakePokemon("thick-fat-mon", types: [plain], abilityName: "thick-fat");
        ...
        {
            Type fireWeak = TestFixtures.MakeType("test-fire-weak", doubleDamageFrom: ["fire"]);

            var pokemon = TestFixtures.MakePokemon(
                "thick-fat-fire-mon",
                types: [fireWeak],
        ...
        {
            Type mixed = TestFixtures.MakeType(
                "test-mixed",
                doubleDamageFrom: ["test-weak"],
                halfDamageFrom: ["test-resist"]
            );

            var pokemon = TestFixtures.MakePokemon("solid-rock-mon", types: [mixed], abilityName: "solid-rock");
        ...
        public void Ability_WonderGuard_OnlyTakesDamageFromGenuineWeaknesses()
        {
            ...
            Type single = TestFixtures.MakeType("test-single", doubleDamageFrom: ["fire"]);

            var pokemon = TestFixtures.MakePokemon("wonder-guard-mon", types: [single], abilityName: "wonder-guard");
